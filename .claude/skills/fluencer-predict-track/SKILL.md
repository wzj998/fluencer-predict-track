...
```powershell
.\.claude\skills\fluencer-predict-track\scripts\ensure-chrome-cdp.ps1
```
...
8. 排除最后编辑时间晚于预测事件、预测过于模糊、关键判断主要依赖不可读图片、或尚未到期且用户未要求列出的内容。
9. 除非可验证候选不足，否则至少选择 10 条，并尽量覆盖不同年份或时间段。
10. 使用外部公开来源验证每条预测，优先官方统计、央行、监管机构、交易所、公司公告、FRED、BIS、World Bank、IMF、NBS、Federal Reserve、CSRC/CFFEX；官方数据不可得时再使用 Yahoo Finance、Stooq、Trading Economics 等可引用行情源。
11. 保存 Markdown 报告：
