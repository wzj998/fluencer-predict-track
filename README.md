...
- 支持把可交易预测转换为回测信号：默认初始本金 1,000,000，按语气强弱分配仓位，尽量获取标的历史数据，使用默认年化无风险利率 2.8% 计算夏普，绘制净值曲线并嵌入 Markdown 报告。
...
三份入口应保持同一套核心规则：每个目标至少 20 条预测样本、多目标分别计数、最后编辑时间检查、外部数据验证、可选回测净值曲线，以及 Markdown 报告中的充分结果总结。
